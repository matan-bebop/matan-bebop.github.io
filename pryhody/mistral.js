...
async function choose(prompt, commands, key)
{
  ...
  const messages = [
    {"role": "system", "content": "Вибери декілька команд зі списку, що найліпше підходять до запиту користувача. Подавай номери найпідходящих команд, пояснення свого вибору й оцінку від 0 до 100. Послідовність дій:\n 1. Спершу виділи основну дію у запиті користувача\n  - Дія з команди має досить точно збігатися з бажанням, яке висловлює користувач у запиті\n 2. Чи є прийменник ('на', 'під', 'у', тощо) у запиті? Якщо є, то: \n  - Бажано, щоб прийменник у запиті збігався з прийменником у команді\n  - Якщо у варіантах немає команди з прийменником, обирай команди з '…' в кінці, якщо замість того '…' можна поставити прийменник із запиту.\n 3. Виділи предмет чи його опис у запиті.\n  - Старайся, щоби предмет у запиті збігався з предметом у команді\n  - Якщо нема команд з підходящим предметом, обирай команди з '…' в кінці, якщо замість того '…' можна поставити предмет з запиту\n 4. Поясни, чому обрана команда відповідає запиту\n  - Якщо в обраній команді інші дієслова, прийменники, предмети чи їх уточнення, поясни свій вибір\n  - Якщо замість '…' в обраній команді треба вписати інформацію з запиту, поясни що ти пропонуєш вписувати.\n 5. Якщо команда не має '…' в кінці і не передає всієї інформації про дію та предмет з запиту, або **не обирай її**, або став низьку оцінку!\n 6. Важливо! Підходящих команд може і не бути: якщо сумніваєшся, став низьку оцінку!"},
    {"role": "user", "content": "Запит: " + prompt},
