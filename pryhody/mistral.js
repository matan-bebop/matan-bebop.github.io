...
choose_and_complete_schema = {
  ...
  "name": "choose_and_complete_schema",
  "strict": true
}}

async function choose_and_complete(prompt, commands, key)
{
  const sys_prompt = "Вибери і доповни декілька команд зі списку, що найліпше підходять до запиту користувача. Подавай номери найпідходящих команд, повну команду, пояснення свого вибору й оцінку від 0 до 100. Послідовність дій:\n 1. Спершу виділи основну дію у запиті користувача\n  - Дія з команди має досить точно збігатися з бажанням, яке висловлює користувач у запиті\n 2. Чи є прийменник ('на', 'під', 'у', тощо) **між дієсловом та предметом** у запиті? Якщо є, то: \n  - Бажано, щоб прийменник у запиті збігався з прийменником **між дієсловом та предметом** у команді\n  - Якщо у варіантах немає команди з прийменником доповни найкращі команди прийменником з запиту.\n 3. Якщо є, виділи предмет чи його опис у запиті.\n  - Старайся, щоби предмет у запиті збігався з предметом у команді\n  - Якщо нема команд з підходящим предметом, доповни найкращі команди предметом з запиту\n 4. Поясни, чому обрана команда відповідає запиту\n  - Якщо в обраній команді інші дієслова, прийменники, предмети чи їх уточнення, поясни свій вибір\n  - Поясни своє доповнення.\n 6. Важливо! Підходящих команд може і не бути: якщо сумніваєшся, став низьку оцінку!"

  return await
    _choose(prompt, sys_prompt, choose_and_complete_schema, commands, key)
}
...
{
  const sys_prompt = "Вибери декілька команд зі списку, що найліпше підходять до запиту користувача. Подавай номери найпідходящих команд, пояснення свого вибору й оцінку від 0 до 100. Послідовність дій:\n 1. Спершу виділи основну дію у запиті користувача. Дія з команди має досить точно збігатися з бажанням, яке висловлює користувач у запиті\n 2. Виділи у запиті є прийменник ('на', 'під', 'у', тощо) **між дієсловлм та предметом**. Бажано, щоби він збігався з прийменником **між дієсловом та предметом** у команді\n 3. Виділи предмет чи його опис у запиті. Старайся, щоби предмет у запиті збігався з предметом у команді\n 4. Поясни, чому обрана команда відповідає запиту. Якщо в обраній команді інші дієслова, прийменники, предмети чи їх уточнення, поясни свій вибір\n 5. Якщо команда не передає всієї інформації про дію та предмет з запиту, або **не обирай її**, або став нижчу оцінку!\n 6. Важливо! Підходящих команд може і не бути: якщо сумніваєшся, став низьку оцінку!"

  return await _choose(prompt, sys_prompt, choose_schema, commands, key)
}

async function _choose(prompt, system_prompt, schema, commands, key)
{
  ...
  const res = await ask_mistral(messages, schema, key)
...
guess_schema = {
  ...
  "json_schema": { "schema": { "properties": {
    "varianty": { "type": "array", "items": {
        ...
        "properties": {
          "komanda": {"type": "string"},
          "perevirka": {"type": "string"},
          "ocinka": {"type": "integer"}
        },
        "required": ["komanda", "perevirka", "ocinka"]
      }
    }
  }},
  "name": "guess_schema",
